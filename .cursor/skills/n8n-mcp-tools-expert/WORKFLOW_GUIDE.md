...
`list` and `get` also accept an optional `includeUsage: true` flag that attaches workflow-usage info to each credential.
...
### Security Notes
- **Response stripping**: `get`, `create`, and `update` all strip the `data` field from responses
- **Log redaction**: Credential request bodies are redacted from debug logs
- **Usage scan resilience**: when `includeUsage: true` triggers a workflow scan that fails, the response includes `usageScanError`
...
- **Remediation Playbook**: Auto-fixable items, items requiring review, items requiring user action
