# Combine extracted keywords with user's existing list

# User's existing negative keywords (from their format)
$existingNegatives = @(
...
foreach ($keyword in $uniqueNegatives) {
    $index++
    $itemsOnLine++

    if ($itemsOnLine -eq 1) {
        $output += "    `"" + $keyword + "`""
    } elseif ($itemsOnLine -le 10) {
        $output += ", `"" + $keyword + "`""
    } else {
        $output += ",`n    `"" + $keyword + "`""
        $itemsOnLine = 0
    ...
    # Add newline every 10 items
    if ($itemsOnLine -eq 0 -and $index -lt $uniqueNegatives.Count) {
        $output += "`n"
    }
}
